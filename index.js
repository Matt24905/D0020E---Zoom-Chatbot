...
const helpFunction = require('./functions/help.js')
const removeInfoFunction = require('./functions/removeInfo.js')
...
app.get('/authorize', async function (req, res) {
  res.send('Thanks for installing the bot for Zoom!')
})
...
chatbot.on('commands', async function (event) {
  ...
  if (event.message.split(' ').length > 1) {
    ...
    else if (secondCommand === 'help') {
      withChatbotToken(helpFunction)
    }
    else if(secondCommand === 'removeInfo') {
      withChatbotToken(removeInfoFunction)
    }
    else if(secondCommand == 'rps') {
      withChatbotToken(rpsFunction)
    }
  }
  ...
  function withChatbotToken(callbackFunction) {
    request({
      ...
      if (error) {
        ...
        else if (secondCommand === 'poll' || secondCommand === 'repeat' || secondCommand === 'coinflip' || secondCommand === 'help' || secondCommand === 'removeInfo' || secondCommand === 'rps') {
...
app.get('/privacy', (req, res) => {
  res.send('The bot for Zoom does not store any user data.')
})
...
app.get('/', (req, res) => {
  res.send('Welcome to the bot for Zoom!')
})
...
app.post('/deauthorize', (req, res) => {
  if (req.headers.authorization === process.env.verification_token) {
    ...
    res.send('Unauthorized request to bot for Zoom.')
  }
})

app.listen(port, () => console.log(`bot for Zoom listening on port ${port}!`))
